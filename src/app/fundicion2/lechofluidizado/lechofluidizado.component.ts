...
@Component({
  selector: 'app-lechofluidizado',
  standalone: true,
  imports: [HighchartsChartModule, CommonModule, HttpClientModule],
  templateUrl: './lechofluidizado.component.html',
  styleUrl: './lechofluidizado.component.scss'
})
export class LechofluidizadoComponent implements OnInit, OnDestroy {
  ...
  lineChartOptions1: Highcharts.Options = this.crearOpcionesGraficoDefault(
    'Temperatura Agua Lecho Fluidizado - Fundición 2',
    'Unidad de medida: Celsius (°C)',
    '°C',
    'Tiempo',
    0,
    40,
  ...
  lineChartOptions2: Highcharts.Options = this.crearOpcionesGraficoDefault(
    'Temperatura Arena Lecho Fluidizado - Fundición 2',
    'Unidad de medida: Celsius (°C)',
    '°C',
    'Tiempo',
    0,
  ...
  lineChartOptions3: Highcharts.Options = this.crearOpcionesGraficoDefault(
    'Presión Bombona Shakeout - Fundición 2',
  ...
  startPolling(intervalMs: number): void {
    const apiUrl = 'http://192.168.1.18:1880/api/f2lechofluidizado'; //EDITAR A GUSTO
      ...
      .subscribe({
        next: (datosProcesados) => {
          ...
          if (this.chart1Instance) {
            ...
            try {
              ...
              if (this.chart1Instance.series && this.chart1Instance.series[1]) {
                ...
                datosProcesados.data4 &&
                datosProcesados.data4.length > 0
              ) {
            ...
            try {
              if (this.chart2Instance.series && this.chart2Instance.series[0]) {
                ...
                this.chart2Instance.series[0].update(
                  {
                    name: 'Temperatura Arena Entrada',
                    type: 'spline',
                ...
                datosProcesados.data5.length > 0
              ) {
                // Si la serie 0 NO existe Y HAY DATOS para ella, la añadimos
                console.log("Añadiendo Serie 1 ('fase A') nueva.");
          ...
          if (this.chart3Instance) {
            ...
            try {
              if (this.chart3Instance.series && this.chart3Instance.series[0]) {
                console.log(
                  "Actualizando datos de Serie 1 ('fase A') existente."
                );
                // this.chart1Instance.series[0].setData(datosProcesados.data1, redibujarPaso);
                this.chart3Instance.series[0].update(
                  {
                    name: 'Presión Bombona',
                    type: 'spline',
              ...
              // --- Manejar Serie 2 (Fase B) ---
              if (this.chart3Instance.series && this.chart3Instance.series[1]) {
                // Si la serie 1 (índice 1) ya existe, se actualizan sus datos
                console.log(
                  "Actualizando datos de Serie 2 ('fase B') existente."
                );
                this.chart3Instance.series[1].setData(
              ...
              if (redibujarAlFinal) {
                console.log('Llamando a chart1Instance.redraw().');
                this.chart3Instance.redraw(); // Redibuja el gráfico con todos los cambios
              }
            } catch (e) {
              console.error(
                'Error durante setData/addSeries en chart1Instance:',
  ...
  fetchGraphData(
    apiUrl: string
  ): Observable<{ data1: any[]; data2: any[]; data3: any[], data4: any[], data5: any[], data6: any[] }> {
    // ... (Tu código existente para fetchGraphData, que ya funciona) ...
    const REQUEST_TIMEOUT = 15000;
    const MEASUREMENT_BOMBONA_SHAKEOUT_PRESION = 'F2-Bombona-Shakeout';
    const MEASUREMENT_TEMP_LECHO = 'F2-Temp-Lecho';

    return this.http.get<any[]>(apiUrl).pipe(
      timeout(REQUEST_TIMEOUT),
      map((datosEnBruto) => {
        const resultado: { data1: any[]; data2: any[]; data3: any[], data4: any[]; data5: any[], data6: any[] } = {
          data1: [],
          data2: [],
          data3: [],
          data4: [],
        ...
        if (Array.isArray(datosEnBruto)) {
          datosEnBruto.forEach((item: any) => {
            ...
            try {
              ...
              if (item.measurement === MEASUREMENT_BOMBONA_SHAKEOUT_PRESION) {
                if (item.field === 'Presion_bombona') {
                  resultado.data1.push([timestamp, valorY]);
                } else if (item.field === 'Presion_entrada') {
                  resultado.data2.push([timestamp, valorY]);
                }
              } else if (item.measurement === MEASUREMENT_TEMP_LECHO) {
                if (item.field === 'Tin_agua') {
                  resultado.data3.push([timestamp, valorY]);
                } else if (item.field === 'Tout_agua') {
                  resultado.data4.push([timestamp, valorY]);
                } else if (item.field === 'Tin_arena') {
                  resultado.data5.push([timestamp, valorY]);
                } else if (item.field === 'Tout_arena') {
      ...
      catchError((error) => {
        ...
        return of({ data1: [], data2: [], data3: [], data4: [], data5: [], data6: [] }); // Devuelve un array vacío en caso de error
      })
    );
  }
}
