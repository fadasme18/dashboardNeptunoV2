...
@Component({
  ...
  styleUrl: './horno3.component.scss'
})
export class Horno3Component implements OnInit, OnDestroy {
  ...
  public estado_horno1F2: Boolean | null = null;
    ...
    lineChartOptions3: Highcharts.Options = this.crearOpcionesGraficoDefault(
      'Horno 3 - energia',
      'Unidad de medida kilo Watts hora (kWh)',
      'Energía (kWh)',
      'Tiempo',
      0,
      4000,
    ...
    lineChartOptions4: Highcharts.Options = this.crearOpcionesGraficoDefault(
      'Horno 3 - potencia',
      'Unidad de medida kilo Watts (kW)',
      'Potencia (kW)',
      'Tiempo',
      0,
      300,
    ...
    private crearOpcionesGraficoDefault(
      ...
      return {
        ...
        legend: {
          ...
          // Opcional: ajustes finos de posición y apariencia si los necesitas:
          // x: 0, // Desplazamiento horizontal desde la posición de 'align'
          // y: 10, // Desplazamiento vertical. Un valor positivo la empuja hacia abajo desde el borde inferior del área de trazado.
          //        //  Highcharts suele manejar bien el espaciado por defecto.
          // floating: false, // Por defecto es false. Si fuera true, flotaría sobre el gráfico.
          // borderWidth: 0, // Por defecto no tiene borde.
          // itemMarginTop: 5, // Espacio encima de cada ítem de la leyenda (útil si hay varias filas)
          // itemMarginBottom: 5, // Espacio debajo
          // itemStyle: {
          //   // fontWeight: 'normal',
          //   // fontSize: '12px'
          // }
            ...
            if (this.points) { 
              this.points.forEach((point) => {
                pointsInfo += `<br/><span style="color:${
                  point.color
                }">\u25CF</span> ${point.series.name}: <b>${(point.y as number)?.toFixed(
                  2
                )} ${umedida}</b>`;
              });
            }
    ...
    startPolling(intervalMs: number): void {
      const apiUrl = 'http://192.168.1.18:1880/api/f1horno3'; //EDITAR A GUSTO
        ...
        .subscribe({
          next: (datosProcesados) => {
            ...
            if (this.chart1Instance) {
              console.log(
                'Actualizando Gráfico 1 usando API de instancia (setData/addSeries)...'
              );
              const redibujarAlFinal = true; // Controlar si redibujamos en cada paso (false) o solo al final (true)
              const redibujarPaso = !redibujarAlFinal; // Si redibujarAlFinal=true, redibujarPaso=false
  
              try {
                if (this.chart1Instance.series && this.chart1Instance.series[0]) {
                  console.log(
                    "Actualizando datos de Serie 1 ('fase A') existente."
                  );
                  // this.chart1Instance.series[0].setData(datosProcesados.data1, redibujarPaso);
                  this.chart1Instance.series[0].update(
                    {
                      name: 'fase A',
                      type: 'spline',
                      id: 'faseA_datos',
                  ...
                  datosProcesados.data1.length > 0
                ) {
                  // Si la serie 0 NO existe Y HAY DATOS para ella, la añadimos
                  console.log("Añadiendo Serie 1 ('fase A') nueva.");
                  this.chart1Instance.addSeries(
                    {
                      type: 'spline',
                      name: 'fase A',
                      data: datosProcesados.data1,
                      yAxis: 0, // Especifica el eje Y si es necesario
                    },
                    redibujarPaso
                  ); // El 'false' evita redibujar aún
                }
  
                // --- Manejar Serie 2 (Fase B) ---
                if (this.chart1Instance.series && this.chart1Instance.series[1]) {
                  // Si la serie 1 (índice 1) ya existe, se actualizan sus datos
                  console.log(
                    "Actualizando datos de Serie 2 ('fase B') existente."
                  );
                  this.chart1Instance.series[1].setData(
                    datosProcesados.data2,
                    redibujarPaso
                  );
                  // this.chart1Instance.series[1].update({ name: 'fase B' }, redibujarPaso);
                } else if (
                  datosProcesados.data2 &&
                  datosProcesados.data2.length > 0
                ) {
                  console.log("Añadiendo Serie 2 ('fase B') nueva.");
                  this.chart1Instance.addSeries(
                    {
                      type: 'spline',
                      name: 'fase B',
                ...
                // --- Manejar Serie 3 (Fase C) ---
                if (this.chart1Instance.series && this.chart1Instance.series[2]) {
                  // Si la serie 2 (índice 2) ya existe, se actualizan sus datos
                  console.log(
                    "Actualizando datos de Serie 3 ('fase C') existente."
                  );
                  this.chart1Instance.series[2].setData(
                    datosProcesados.data3,
                    redibujarPaso
                  );
                  // this.chart1Instance.series[2].update({ name: 'fase C' }, redibujarPaso);
                } else if (
                  datosProcesados.data3 &&
                  datosProcesados.data3.length > 0
                ) {
                  console.log("Añadiendo Serie 3 ('fase C') nueva.");
                  this.chart1Instance.addSeries(
                    {
                      type: 'spline',
                      name: 'fase C',
                ...
                if (redibujarAlFinal) {
                  console.log('Llamando a chart1Instance.redraw().');
                  this.chart1Instance.redraw(); // Redibuja el gráfico con todos los cambios
                }
              } catch (e) {
                console.error(
                  'Error durante setData/addSeries en chart1Instance:',
                  e
                );
              }
            } else {
              // --- Plan B: Si la instancia aún no existe (puede ser en la primera carga) ---
              // se actualiza las opciones como antes, para que se usen cuando se cree el gráfico
              console.warn(
                'chart1Instance no disponible, actualizando opciones...'
              );
              this.lineChartOptions1 = {
                ...this.lineChartOptions1,
                series: [
                  // se definen las 3 series aquí para la creación inicial
                  {
                    type: 'spline',
                    name: 'fase A',
                    data: datosProcesados.data1,
            ...
            // --- FIN: Actualización Directa para Gráfico 1 ---
            // --- ACTUALIZAR GRAFICOS TAL CUAL SI SE REQUIERE MAS DE UN DATO POR GRAFICO, SI NO, DEJAR COMO CONTINUA ---
            ...
            if (this.chart2Instance) {
              ...
              try {
                if (this.chart2Instance.series && this.chart2Instance.series[0]) {
                  console.log(
                    "Actualizando datos de Serie 1 ('fase A') existente."
                  );
                  // this.chart1Instance.series[0].setData(datosProcesados.data1, redibujarPaso);
                  this.chart2Instance.series[0].update(
                    {
                      name: 'fase A',
                      type: 'spline',
                      id: 'faseA_datos',
                      data: datosProcesados.data6,
                    },
                    redibujarPaso
                  );
                } else if (
                  datosProcesados.data6 &&
                  datosProcesados.data6.length > 0
                ) {
                  // Si la serie 0 NO existe Y HAY DATOS para ella, la añadimos
                  console.log("Añadiendo Serie 1 ('fase A') nueva.");
                  this.chart2Instance.addSeries(
                    {
                      type: 'spline',
                      name: 'fase A',
                      data: datosProcesados.data6,
                ...
                // --- Manejar Serie 2 (Fase B) ---
                if (this.chart2Instance.series && this.chart2Instance.series[1]) {
                  // Si la serie 1 (índice 1) ya existe, se actualizan sus datos
                  console.log(
                    "Actualizando datos de Serie 2 ('fase B') existente."
                  );
                  this.chart2Instance.series[1].setData(
                    datosProcesados.data7,
                    redibujarPaso
                  );
                  // this.chart1Instance.series[1].update({ name: 'fase B' }, redibujarPaso);
                } else if (
                  datosProcesados.data7 &&
                  datosProcesados.data7.length > 0
                ) {
                  console.log("Añadiendo Serie 2 ('fase B') nueva.");
                  this.chart2Instance.addSeries(
                    {
                ...
                // --- Manejar Serie 3 (Fase C) ---
                if (this.chart2Instance.series && this.chart2Instance.series[2]) {
                  // Si la serie 2 (índice 2) ya existe, se actualizan sus datos
                  console.log(
                    "Actualizando datos de Serie 3 ('fase C') existente."
                  );
                  this.chart2Instance.series[2].setData(
                    datosProcesados.data8,
                    redibujarPaso
                  );
                  // this.chart2Instance.series[2].update({ name: 'fase C' }, redibujarPaso);
                } else if (
                  datosProcesados.data8 &&
                  datosProcesados.data8.length > 0
                ) {
                  console.log("Añadiendo Serie 3 ('fase C') nueva.");
                  this.chart2Instance.addSeries(
                    {
                ...
                if (redibujarAlFinal) {
                  console.log('Llamando a chart1Instance.redraw().');
                  this.chart2Instance.redraw(); // Redibuja el gráfico con todos los cambios
                }
              } catch (e) {
                console.error(
                  'Error durante setData/addSeries en chart1Instance:',
            ...
            this.lineChartOptions4 = {
              ...
              series: [
                {
                  type: 'spline',
                  name: this.lineChartOptions4.title?.text,
                  data: datosProcesados.data5,
                },
              ],
            };
            ...
            setTimeout(() => {
              ...
              [
                ...
                datosProcesados.data2,
                datosProcesados.data3,
                datosProcesados.data4,
                datosProcesados.data5,
                datosProcesados.data6,
                datosProcesados.data7,
                datosProcesados.data8
              ].forEach((dataSet) => {
              ...
              if (overallMaxTimestamp > 0) {
                ...
                this.chart4Instance?.xAxis?.[0]?.setExtremes(
                  nuevoMin,
                  nuevoMax,
                  necesitaRedibujar,
                  animacion
    ...
    fetchGraphData(
      apiUrl: string
    ): Observable<{ data1: any[]; data2: any[]; data3: any[]; data4: any[]; data5: any[]; data6: any[]; data7: any[]; data8: any[] }> {
      const REQUEST_TIMEOUT = 15000;
      const MEDICION_OBJETIVO = 'F1-Horno3-[COUNTIS-E53]'; // CAMBIAR NOMBRE AL MEASUREMENT CORRESPONDIENTE
      ...
      return this.http.get<any[]>(apiUrl).pipe(
              ...
              try {
                ...
                switch (item.field) {
                  ...
                  case 'Current3':
                    ...
                    break;
                  case 'Energy':
                    resultado.data4.push([timestamp, valorY]);
                    break;
                  case 'Power':
                    resultado.data5.push([timestamp, valorY]);
                    break;
                  case 'Voltage1':
                    resultado.data6.push([timestamp, valorY]);
                    break;
                  case 'Voltage2':
                    resultado.data7.push([timestamp, valorY]);
                    break;
                  case 'Voltage3':
                    resultado.data8.push([timestamp, valorY]);
                    break;
                  default:
                    break;
                }
        ...
        catchError((error) => {
          ...
          return of({ data1: [], data2: [], data3: [], data4: [], data5: [], data6: [], data7: [], data8: []});
        })
      );
    }
}